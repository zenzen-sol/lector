  ...
  return (
    <div className='flex py-2 hover:bg-gray-50 flex-col cursor-pointer' onClick={onClick}>
      ...
      <div className='flex items-center gap-4 flex-shrink-0 text-sm text-gray-500'>
        {!result.isExactMatch && <span>{(result.score * 100).toFixed()}% match</span>}
  ...
  return (
    <div className='space-y-2'>
      ...
      <div className='divide-y divide-gray-100'>
        {results.map((result) => (
          <ResultItem key={`${result.pageNumber}-${result.matchIndex}`} result={result} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      {results.hasMoreResults && (
        <div className='text-center text-sm text-blue-500'>More results available</div>
      )}
    </div>
  );
};
